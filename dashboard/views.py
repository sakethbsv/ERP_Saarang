...
from erp_test.dashboard.models import *
from erp_test.dashboard.forms import *
from erp_test.users.models import *
from erp_test.users.views import view_profile
...
def display_contacts (request , owner_name=None):#this will be a common tab 
    ...
    for dept_name in dept_names:
        core_profiles = userprofile.objects.filter (
            user__group__label__name = 'Department',
            user__group__name = dept_name,
            user__groups__name = 'Cores')
        coord_profiles = userprofile.objects.filter (
            user__group__label__name = 'Department',
            user__group__name = dept_name,
...
"""
@needs_authentication
def change_profile_pic(request ,owner_name=None):
...
"""
@needs_authentication
def shout(request ,owner_name=None):
    if request.method=="POST":
...
def test(request ,owner_name=None):
    print "in test function"
    return render_to_response('dashboard/photo_list.html',{})
...
@needs_authentication    
def display_calendar2(request ,owner_name=None):
    ...
    curr_month_name=month_arr[month-1]
    ...
    calendar_data=[[{'subtask':[],'date':0} for x in range(0,7)]for y in range(0,6)]
    ...
    for index ,sub in enumerate(user_tasks):
	    ...
	    if  sub.deadline.month==month:
	    	calendar_data[pos_to_task/7][pos_to_task%7]['subtask'].append(sub)
	except: 
            print"some of the tasks dont have their date field assigned"
#    return calendar_data
    return render_to_response('dashboard/mycalendar2.html',locals() ,context_instance = global_context(request))
...
def display_calendar(request ,owner_name=None , month=0 ,year=0):
  ...
  #declaration
    main_data=[[{'subtask':[],'date':0} for x in range(0,7)]for y in range(0,6)]
    
    for index1 ,row in enumerate(mycalendar):
        for index2 ,col in enumerate(row):
    	    main_data[index1][index2]['date']=col
    ...
    for index ,sub in enumerate(user_tasks):
        try:
            pos_to_task=int(sub.deadline.day+first_week_day)-1
	    if  sub.deadline.month==month:
	    	main_data[pos_to_task/7][pos_to_task%7]['subtask'].append(sub)
	except: 
            print"some of the tasks dont have their date field assigned"
	    
    #today_task=complete_data[today+first_week_day-1]#the task of today
