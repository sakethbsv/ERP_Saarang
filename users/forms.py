...
class userprofileForm (ModelForm):
	...
	if (self.cleaned_data['room_no']>1000 or self.cleaned_data['room_no']<100):
	    raise forms.ValidationError(u'Please enter a valid Room No')
	else:
	    return self.cleaned_data['room_no']
	    	

    def clean_summer_stay(self):
	if not self.cleaned_data['summer_stay'].replace(' ','').isalpha():
	    raise forms.ValidationError(u'Please enter a valid summer stay place.')
	else:
	    return self.cleaned_data['summer_stay']
...
class invite_coord(forms.Form):
    ...
    def clean_name (self):
        ...
        name_submitted = self.cleaned_data['name']
        if given_name == '':
...
class Change_password(forms.Form):
     new_password=models.CharField(min_length = 6,max_length=100)     
